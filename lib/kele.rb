...
class Kele
  ...
  def create_message(sender, recipient_id, token, subject, message)
    response = self.class.post("/messages", query: {"sender": sender, "recipient_id": recipient_id, "token": token, "subject": subject, "stripped-text" => message }, headers: { "authorization": @auth_token })
  end
end
